...
### FR-AUTH-006 Временная регистрация/авторизация по login/password
- Метод: `POST /api/v1/auth/temp-register`
- Вход: `login`, `password`.
- Правила login:
- минимум 3 символа;
...
- Поведение:
- если пользователя с таким `login` нет, создать пользователя и вернуть токены (`access_token`, `refresh_token`);
- если пользователь уже существует, вернуть `409 conflict`.
- Ошибка валидации login: `400 bad_request`.
...
### AC-005 Smoke Flow Temp Register
1. `POST /api/v1/auth/temp-register` с новым `login` создает пользователя.
2. Ответ содержит `user_id`, `access_token`, `refresh_token`.
3. Повторный запрос с тем же `login` возвращает `409`.
